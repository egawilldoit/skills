---
name: loop-on-ci
description: "Watch a pull request's checks and iterate on failures until all required checks are green, using gh pr checks as the source of truth for PR-attached checks. Use when a PR must be driven to green across repeated pushes. For a single failing check, use fix-ci."
---
