...
Companion to `understand-codebase` and `recover-design-rationale`. `understand-codebase` tells you what the code does. `recover-design-rationale` tells you why it's shaped that way. Blast radius tells you what it breaks somewhere else.
...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Use `recover-design-rationale` step 2 to pull the PR and commits.
...
4. Be honest about each risk. Give it a real chance of happening and a real cost if it does. Keep the risks you confirmed. List the ones you checked and cleared separately. Same rules as `recover-design-rationale`. Cite a real `file:line`, a search that finds nothing is still an answer, and never make up a caller or an API.
5. Prove the one fact. Write a script or test that runs the real code, run it, and paste what happened.
6. For a big or wide change, run it as an `compare-designs`. Ask several models the same question and merge the answers. Different models catch different real bugs.
